...
curveintegral_settings = import_bokeh('curveintegral_settings.py')
...
function_controls = VBoxForm(
    children=[u_input, v_input, VBox(width=ww, height=20)],
    width=ww)
curve_controls = VBoxForm(
    children=[cx_input, cy_input, parameter_input, VBox(width=ww, height=10)],
    width=ww)
...
# Add panels to tabs
tabs = VBox(
    children=[Tabs(tabs=[function_panel, curve_panel])],
    width=ww)

# refresh quiver field all 100ms
curdoc().add_periodic_callback(refresh_quiver, 100)
# make layout
curdoc().add_root(VBoxForm(children=[HBox(children=[VBox(children=[plot_field, plot_integral]),
                                                    tabs])]))
